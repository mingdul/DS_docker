...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2) {
	...
	//ll2 의 노드를 ll1에 한칸 건너서 삽입
	//ll1이 짧아서 삽입 위치가 없으면 남은 ll2노드 유지
	//삽입된 노드는 ll2에서 제거되며 ll1의 사이즈는 증가하고 ll2는 감소

	while (curr1 != NULL && curr2 !=NULL){
		next1=curr1->next; //ll1 다음 노드 저장
		next2=curr2->next; //ll2 다음노드 저장

		//ll2의 현재노드를 ll1의 다음 위치에 삽입
		curr1->next=curr2;
		curr2->next=next1;
		...
		// ll2->head -> item = ll1 -> head -> next;
		// curr2->item=curr1->next->item;
